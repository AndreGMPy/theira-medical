      ...
      <div className="mx-auto max-w-[1160px] px-4 sm:px-[22px] lg:px-7">
        <div className="flex flex-col gap-4 border-b border-[var(--color-beige)] pb-6 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-2xl">
            <p className="text-[0.74rem] font-semibold uppercase tracking-[0.12em] text-[var(--color-terracotta)]">El universo Theira</p>
            ...
            <p className="mt-3 text-[0.94rem] leading-6 text-[var(--color-muted)]">Explora las categorías de Medicina Estética, Regenerativa y Antienvejecimiento. Cada protocolo parte de una valoración personalizada.</p>
          </div>
        ...
        <div className="mt-7 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {catalogCategories.map((category, index) => (
            <article key={category.id} className="group flex min-h-[292px] flex-col overflow-hidden rounded-[20px] border border-[var(--color-beige)] bg-[var(--color-ivory)]">
              ...
              <div className="flex flex-1 flex-col p-[18px]">
                ...
                <p className="mt-2 line-clamp-2 text-[0.84rem] leading-5 text-[var(--color-muted)]">{category.description}</p>
